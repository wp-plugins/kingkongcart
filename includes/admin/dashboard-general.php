...
if(sanitize_text_field( $_POST['form_status'] )){
	...
	if(sanitize_text_field( $_POST['id_order'] )){
		update_option("kingkongcart_mypage_order", sanitize_text_field( $_POST['id_order'] ));
	}
	...
	if(sanitize_text_field( $_POST['secret_key'] )){
		update_option("kingkongcart_update_key", sanitize_text_field( $_POST['secret_key'] ));
	}
...
<form method="POST">
	...
	<table style="padding-top:10px; padding-left:10px">
		...
		<tr style="height:40px">
			<td>업데이트 인증키</td>
			<td><input type="text" name="secret_key" value="<?php echo $secret_key;?>"></td>
		</tr>
		<tr>
			<td colspan="2" style="height:80px; vertical-align:top">
				<span style="color:red">
				<b>아이띰쏘(www.ithemeso.com) 에서 발급받은 인증키를 입력하시기 바랍니다.</b><br>
				인증키를 입력하지 않으면 플러그인 업데이트 버전이 있어도 체크 및 다운로드가 되지 않습니다.<br>
				</span>
				인증키를 발급 하시지 않았다면 http://www.ithemeso.com 에서 인증키를 발급 받으시기 바랍니다.
			</td>
		</tr> 
		<tr style="height:40px">
